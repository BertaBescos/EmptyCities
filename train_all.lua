...
opt = {
	...
	epoch_synth = 50,	   	-- train with real and synthetic data from this epoch on
...
local data_loader = paths.dofile('data/data_all.lua')
...
if opt.NSYTNH_DATA_ROOT ~= '' then
	 opt.phase = 'train'
	 nsynth_data_loader = paths.dofile('data/data_all.lua') --bbescos
...
if opt.gpu > 0 then
	...
	netD:cuda(); netG:cuda(); netDynSS:cuda() 
	criterion:cuda(); criterionAE:cuda(); criterionSS:cuda()
	print('done')
else
